...
public class Searcher {
  ...
  private final IndexSearcher seeker;

  public Searcher(String indexDir) throws IOException {

    Directory index = FSDirectory.open(new File(indexDir).toPath());
    seeker = new IndexSearcher(DirectoryReader.open(index));
  }
  ...
  public List<Result> search(String searchQuery, AdvancedSearch options) throws IOException, ParseException, InvalidTokenOffsetsException {
    ...
    if (options.getSortBy().equals("relevance"))
      topDocs = seeker.search(query, 48, Sort.RELEVANCE);
    else
      topDocs = seeker.search(query, 48, new Sort(new SortedNumericSortField(Constants.SORTBY_DATE, SortField.Type.LONG, true)));
    ...
    for (ScoreDoc sd : topDocs.scoreDocs) {
      ...
      Document doc = seeker.doc(sd.doc);
